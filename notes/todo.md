...
Within those two groups, packages are sorted by path. Within each package, items are grouped using the branch types from `.pi/prompts/implement-plan-item.md`: `refactor`, `fix`, `feature`, `docs`, `test`, and `chore`, in that order. Empty type sections are omitted.
