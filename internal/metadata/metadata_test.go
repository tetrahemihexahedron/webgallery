	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(got.FileProblems) != 0 {
				t.Errorf("metadata.Read(%q) returned file problems: %v", path, got.FileProblems)
			}
...
func TestReadDirectories(t *testing.T) {
	t.Run("empty directory", func(t *testing.T) {
		...
		if len(got.Metadata) != 0 {
			t.Errorf("metadata.Read(%q) returned metadata: %+v", path, got.Metadata)
		}
		if len(got.FileProblems) != 0 {
			t.Errorf("metadata.Read(%q) returned file problems: %+v", path, got.FileProblems)
		}
	...
	t.Run("directory with mixed results", func(t *testing.T) {
		...
		if len(got.FileProblems) != len(wantProblems) {
			t.Fatalf("metadata.Read(%q) returned %d file problems, want %d: %+v", path, len(got.FileProblems), len(wantProblems), got.FileProblems)
		}
		for i, want := range wantProblems {
			problem := got.FileProblems[i]
			if problem.FileName != want.fileName {
				t.Errorf("metadata.Read(%q) problem %d filename = %q, want %q", path, i, problem.FileName, want.fileName)
			}
			if !strings.Contains(problem.Message, want.message) {
				t.Errorf("metadata.Read(%q) problem %d message = %q, want message containing %q", path, i, problem.Message, want.message)
			}
		}
	})
}

func TestReadReportsFileProblems(t *testing.T) {
	tests := []struct {
		name         string
		file         string
		wantFileName string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(got.FileProblems) != 1 {
				t.Fatalf("metadata.Read(%q) returned %d file problems, want 1: %+v", path, len(got.FileProblems), got.FileProblems)
			}
			problem := got.FileProblems[0]
			if problem.FileName != tc.wantFileName {
				t.Errorf("metadata.Read(%q) problem filename = %q, want %q", path, problem.FileName, tc.wantFileName)
			}
			if !strings.Contains(problem.Message, tc.wantMessage) {
				t.Errorf("metadata.Read(%q) problem message = %q, want message containing %q", path, problem.Message, tc.wantMessage)
			}
		})
	}
}
...
func TestReadReturnsError(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatalf("metadata.Read(%q) returned nil error, want error wrapping *exec.ExitError", path)
			}
